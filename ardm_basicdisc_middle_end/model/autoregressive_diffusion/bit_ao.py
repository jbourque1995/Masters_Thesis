...
from ardm_basicdisc_middle_end.model.autoregressive_diffusion import ardm_utils
from ardm_basicdisc_middle_end.model.autoregressive_diffusion import transitions

from ardm_basicdisc_middle_end.utils import distribution_utils
from ardm_basicdisc_middle_end.utils import dynamic_programming
...
class BitUpscaleAutoregressiveDiffusion(struct.PyTreeNode):
  ...
  def elbo_with_features(self, rng, params, x, train, context=None):
    ...
    Returns the normal ARDM loss terms along with two feature dictionaries:
    `fake_features` are produced from the normal partially observed ARDM input;
    `real_features` are produced from the fully observed target at the same
    timestep/stage. The discriminator heads compare these internal
    representations rather than changing the final output tensor directly.
    """
    d = np.prod(x.shape[1:])
    batch_size = x.shape[0]
    ...
    # Fully observed pass used only to provide a same-shape internal reference
    # representation for the feature discriminator. Its features are stop-grad'ed
    # in train.py before discriminator training.
    fully_observed = jnp.ones_like(already_predicted)
    real_net_inp = x_future
    _, real_features = self.apply_fn(
        {'params': params}, real_net_inp, t,
        self.prepare_additional_input(stage, fully_observed), train,
        context, return_features=True,
        rngs={'dropout': rng_drop_real} if train else None)
    ...
    return elbo, elbo_per_t, ce, t, fake_features, real_features
