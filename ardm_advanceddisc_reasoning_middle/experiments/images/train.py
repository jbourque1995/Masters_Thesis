...
from ardm_advanceddisc_reasoning_middle.experiments.images import metric_logging
from ardm_advanceddisc_reasoning_middle.experiments.images.architectures import discriminator
...
from ardm_advanceddisc_reasoning_middle.experiments.images import checkpoint
from ardm_advanceddisc_reasoning_middle.experiments.images import custom_train_state
from ardm_advanceddisc_reasoning_middle.experiments.images import datasets
from ardm_advanceddisc_reasoning_middle.experiments.images.architectures import unet
from ardm_advanceddisc_reasoning_middle.model.autoregressive_diffusion import ao_arm
from ardm_advanceddisc_reasoning_middle.model.autoregressive_diffusion import bit_ao
...
def train_step(rng, batch, state, disc_mid_state, disc_end_state, epoch, model,
  ...
  (model_rng, gen_mid_dropout_rng, gen_end_dropout_rng,
   ...
   disc_end_real_dropout_rng, disc_end_fake_dropout_rng) = jax.random.split(rng, 7)
  ...
  def loss_fn(params):
    ...
    if use_mid:
      fake_mid_logits = _apply_discriminator(
          ...
          train=True, rng=gen_mid_dropout_rng)
    ...
    if use_end:
      fake_end_logits = _apply_discriminator(
          ...
          train=True, rng=gen_end_dropout_rng)
    ...
    loss = (ardm_loss
            + disc_weight_mid * gen_mid_loss
            + disc_weight_end * gen_end_loss
            + reason_weight_mid * reason_mid_loss
            + reason_weight_end * reason_end_loss)
    aux = (ardm_loss, gen_mid_loss, gen_end_loss, reason_mid_loss,
           reason_end_loss, disc_weight_mid, disc_weight_end,
           reason_weight_mid, reason_weight_end, guidance_progress, elbo_value,
           elbo_per_t, ce_value, t, fake_features, real_features,
           feature_info)
    return loss, aux

  grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
  (loss, (ardm_loss, gen_mid_loss, gen_end_loss, reason_mid_loss,
          reason_end_loss, disc_weight_mid, disc_weight_end, reason_weight_mid,
          reason_weight_end, guidance_progress, elbo_value, elbo_per_t,
          ce_value, t, fake_features, real_features, feature_info)), grads = (
