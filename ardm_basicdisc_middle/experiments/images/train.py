...
from ardm_basicdisc_middle.experiments.images import metric_logging
from ardm_basicdisc_middle.experiments.images.architectures import discriminator
...
from ardm_basicdisc_middle.experiments.images import checkpoint
from ardm_basicdisc_middle.experiments.images import custom_train_state
from ardm_basicdisc_middle.experiments.images import datasets
from ardm_basicdisc_middle.experiments.images.architectures import unet
from ardm_basicdisc_middle.model.autoregressive_diffusion import ao_arm
from ardm_basicdisc_middle.model.autoregressive_diffusion import bit_ao
from ardm_basicdisc_middle.utils import util_fns
...
def train_step(rng, batch, state, disc_mid_state, disc_end_state, epoch, model,
  ...
  def loss_fn(params):
    elbo_value, elbo_per_t, ce_value, t, fake_features, real_features = (
        model.elbo_with_features(rng, params, batch['image'], train=True))
    ...
    if use_mid:
      fake_mid_logits = disc_mid_model.apply(
          {'params': disc_mid_state.params}, fake_features['middle'],
          train=True)
      gen_mid_loss = generator_hinge_loss(fake_mid_logits)
    if use_end:
      fake_end_logits = disc_end_model.apply(
          {'params': disc_end_state.params}, fake_features['end'], train=True)
      gen_end_loss = generator_hinge_loss(fake_end_logits)

    warmup_done = epoch >= config.disc_warmup_epochs
    disc_weight_mid = jnp.where(warmup_done, config.disc_weight_mid, 0.0)
    disc_weight_end = jnp.where(warmup_done, config.disc_weight_end, 0.0)
    loss = ardm_loss + disc_weight_mid * gen_mid_loss + disc_weight_end * gen_end_loss
    aux = (ardm_loss, gen_mid_loss, gen_end_loss, disc_weight_mid,
           disc_weight_end, elbo_value, elbo_per_t, ce_value, t,
           fake_features, real_features)
    return loss, aux

  grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
  (loss, (ardm_loss, gen_mid_loss, gen_end_loss, disc_weight_mid,
          disc_weight_end, elbo_value, elbo_per_t, ce_value, t,
          fake_features, real_features)), grads = grad_fn(state.params)
  ...
  if use_mid:
    def disc_mid_loss_fn(disc_params):
      real_logits = disc_mid_model.apply(
          {'params': disc_params}, jax.lax.stop_gradient(real_features['middle']),
          train=True)
      fake_logits = disc_mid_model.apply(
          {'params': disc_params}, jax.lax.stop_gradient(fake_features['middle']),
          train=True)
      return discriminator_hinge_loss(real_logits, fake_logits), (real_logits, fake_logits)
  ...
  if use_end:
    def disc_end_loss_fn(disc_params):
      real_logits = disc_end_model.apply(
          {'params': disc_params}, jax.lax.stop_gradient(real_features['end']),
          train=True)
      fake_logits = disc_end_model.apply(
          {'params': disc_params}, jax.lax.stop_gradient(fake_features['end']),
          train=True)
      return discriminator_hinge_loss(real_logits, fake_logits), (real_logits, fake_logits)
...
def train_and_evaluate(config,
  ...
  dummy_mid_features = jnp.ones((1, mid_channels), dtype=jnp.float32)
  dummy_end_features = jnp.ones((1, end_channels), dtype=jnp.float32)
  disc_mid_variables = disc_mid_model.init(disc_mid_init_rng, dummy_mid_features, train=True)
  disc_end_variables = disc_end_model.init(disc_end_init_rng, dummy_end_features, train=True)
  disc_tx = optax.adam(
