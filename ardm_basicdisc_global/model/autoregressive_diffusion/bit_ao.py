...
# ==============================================================================
# Modifications made by Jordan Bourque for honors thesis research.
# University of New Brunswick, 2025.
# Purpose: Integrate a GAN-style discriminator into ARDM training and evaluation.
# ==============================================================================
...
from ardm_basicdisc_global.model.autoregressive_diffusion import ardm_utils
from ardm_basicdisc_global.model.autoregressive_diffusion import transitions

from ardm_basicdisc_global.utils import distribution_utils
from ardm_basicdisc_global.utils import dynamic_programming
...
class BitUpscaleAutoregressiveDiffusion(struct.PyTreeNode):
  ...
  def elbo_with_fake(self, rng, params, x, train, context=None):
    d = np.prod(x.shape[1:])
    batch_size = x.shape[0]
    ...
    fake_img = self.make_fake_image_from_net_out(
        net_out, x_future, x_past, stage, already_predicted, to_predict)

    return elbo, elbo_per_t, ce, t, fake_img

  def make_fake_image_from_net_out(self, net_out, x_future, x_past, stage,
                                   already_predicted, to_predict):
    """Creates a differentiable image approximation from the network output."""
    batch_size = x_past.shape[0]
    ...
    if self.direct_parametrization:
      ...
      logits = logits_per_stage[jnp.arange(batch_size), Ellipsis, stage, :]
      probs = jax.nn.softmax(logits, axis=-1)
      values = jnp.arange(self.branch_factor, dtype=jnp.float32)
      expected_digit = jnp.sum(probs * values, axis=-1)

      t = self.num_stages - 1 - stage
      t = jnp.expand_dims(t, np.arange(1, len(x_past.shape)))
      expected_future = x_past.astype(jnp.float32) + (
          expected_digit * self.branch_factor**t)

    else:
      logits = net_out.reshape(*x_past.shape, self.num_output_classes)
      log_probs = self.log_prob_vector_future_given_past(logits, x_past, stage)
      probs = jnp.exp(log_probs)
      values = jnp.arange(self.num_input_classes, dtype=jnp.float32)
      expected_future = jnp.sum(probs * values, axis=-1)

    fake_img = (already_predicted.astype(jnp.float32) *
                x_future.astype(jnp.float32) +
                to_predict.astype(jnp.float32) * expected_future)
    fake_img = jnp.clip(fake_img, 0.0, float(self.num_output_classes - 1))
    fake_img = fake_img / float(self.num_output_classes - 1)
    fake_img = fake_img * 2.0 - 1.0
    return fake_img
  ...
  def create(cls, config, get_architecture):
    """Creates a new instance with initialized parameters."""
    ...
    if config.upscale_direct_parametrization:
        ...
        if config.upscale_mode == 'zero_least_significant':
            ...
            raise ValueError(f"Unsupported upscale_mode: {config.upscale_mode}")
    ...
    neural_net = get_architecture(num_input_classes, required_num_outputs, num_steps)

    return cls(
        config=config,  # keep this as-is
