...
"""Backfill FID / IS / SSIM for saved ARDM checkpoints.
...
from ardm_basicdisc_global.experiments.images import train
...
def _make_initial_state(config):
  ...
  # Needed so config.data_shape and config.num_classes match training.
...
def main(argv):
  ...
  # Resolve generated_eval and metrics to be local to this run folder,
  # while keeping real_image_dir shared.
  ...
  for ckpt_num in ckpt_nums:
    ...
    logging.info("Sampling %d images from ckpt_%d",
                 config.num_samples, ckpt_num)
