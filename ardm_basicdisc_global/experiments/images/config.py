...
def get_config():
  ...
  config.num_epochs = 200        # was 50, was 6000
  ...
  config.disc_weight = 0.0005                # How strongly discriminator affects training loss
  config.disc_warmup_epochs = 6
  config.disc_start_epoch = 6  # compatibility with newer configs; older code ignores this
  config.disc_base_channels = 64
  config.disc_learning_rate = 0.00001
  config.disc_beta1 = 0.5
  config.disc_beta2 = 0.999
  config.max_eval_batches = 20
  ...
  # Architecture matched to baseline/run_ardm_original_changed_res_blocks
  config.architecture = D(
      n_channels=64,            # was 256
      num_res_blocks=2,
      num_heads=1,
      ch_mult=[1, 2, 2, 2],
