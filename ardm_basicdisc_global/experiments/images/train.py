...
from ardm_basicdisc_global.experiments.images import metric_logging
from ardm_basicdisc_global.experiments.images.architectures import discriminator
...
from ardm_basicdisc_global.experiments.images import checkpoint
from ardm_basicdisc_global.experiments.images import custom_train_state
from ardm_basicdisc_global.experiments.images import datasets
from ardm_basicdisc_global.experiments.images.architectures import unet
from ardm_basicdisc_global.model.autoregressive_diffusion import ao_arm
from ardm_basicdisc_global.model.autoregressive_diffusion import bit_ao
from ardm_basicdisc_global.utils import util_fns
...
def train_step(rng, batch, state, disc_state, epoch, model, disc_model, config):
  ...
  real_img = batch['image'].astype(jnp.float32) / float(config.num_classes - 1)
  real_img = real_img * 2.0 - 1.0

  def loss_fn(params):
    elbo_value, elbo_per_t, ce_value, t, fake_img = model.elbo_with_fake(
        rng_gen, params, batch['image'], train=True)
    ardm_loss = -elbo_value.mean(0) - config.ce_term * ce_value.mean(0)

    fake_logits_for_gen = disc_model.apply(
        {'params': disc_state.params}, fake_img, train=True)
    gen_adv_loss = generator_hinge_loss(fake_logits_for_gen)

    disc_weight = jnp.where(
        epoch < config.disc_warmup_epochs, 0.0, config.disc_weight)
    loss = ardm_loss + disc_weight * gen_adv_loss
    return loss, (ardm_loss, gen_adv_loss, disc_weight, elbo_value,
                  elbo_per_t, ce_value, t, fake_img)

  grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
  (loss, (ardm_loss, gen_adv_loss, disc_weight, elbo_value, elbo_per_t,
          ce_value, t, fake_img)), grads = grad_fn(state.params)
  ...
  def disc_loss_fn(disc_params):
    fake_img_detached = jax.lax.stop_gradient(fake_img)
    real_logits = disc_model.apply(
        {'params': disc_params}, real_img, train=True)
    fake_logits = disc_model.apply(
        {'params': disc_params}, fake_img_detached, train=True)
    disc_loss = discriminator_hinge_loss(real_logits, fake_logits)
    return disc_loss, (real_logits, fake_logits)

  disc_grad_fn = jax.value_and_grad(disc_loss_fn, has_aux=True)
  (disc_loss, (real_logits, fake_logits)), disc_grads = disc_grad_fn(
      disc_state.params)
  disc_grads = jax.lax.pmean(disc_grads, axis_name='batch')
  disc_grad_norm = util_fns.global_norm(disc_grads)
  disc_state = disc_state.apply_gradients(grads=disc_grads)

  metrics = {
      'loss': jax.lax.pmean(loss, axis_name='batch'),
      'ardm_loss': jax.lax.pmean(ardm_loss, axis_name='batch'),
      'gen_adv_loss': jax.lax.pmean(gen_adv_loss, axis_name='batch'),
      'disc_loss': jax.lax.pmean(disc_loss, axis_name='batch'),
      'disc_real_logits': jax.lax.pmean(jnp.mean(real_logits), axis_name='batch'),
      'disc_fake_logits': jax.lax.pmean(jnp.mean(fake_logits), axis_name='batch'),
      'disc_weight': jax.lax.pmean(disc_weight, axis_name='batch'),
  ...
  return state, disc_state, metrics, rng_return
...
def train_epoch(p_train_step, state, disc_state, train_ds, batch_size, epoch, rng,
                kl_tracker, config):
  ...
  with jax.profiler.StepTraceAnnotation('train', step_num=state.step):
    for batchnum, batch in enumerate(train_ds):
      state, disc_state, metrics, rng = p_train_step(rng, batch, state,
                                                     disc_state, epoch)

      # Pull only this batch's metrics back to CPU immediately. This avoids
      # keeping thousands of device arrays alive until the end of the epoch.
      metrics = jax.device_get(flax.jax_utils.unreplicate(metrics))

      if getattr(config, 'track_kl', False):
        t_batch = metrics['t_batch'].reshape(batch_size)
        nelbo_per_t_batch = metrics['nelbo_per_t_batch'].reshape(batch_size)
      ...
      if batchnum % 500 == 0:
        print(f'epoch {epoch}, batch {batchnum}', flush=True)

  # Compute mean of metrics across each batch in epoch.
  epoch_metrics = {
      key: np.mean([metrics[key] for metrics in scalar_metrics])
      for key in scalar_metrics[0]}
  ...
  return state, disc_state, epoch_metrics, rng
...
def log_standard_metrics(writer, train_metrics, eval_metrics, epoch):
  ...
  for key in ('ardm_loss', 'gen_adv_loss', 'disc_loss', 'disc_real_logits',
              'disc_fake_logits', 'disc_weight', 'disc_grad_norm'):
...
def train_and_evaluate(config,
  ...
  rng, disc_init_rng = jax.random.split(rng)

  disc_model = discriminator.ConvDiscriminator(
      base_channels=config.disc_base_channels)

  dummy_img = jnp.ones((1, *config.data_shape), dtype=jnp.float32)

  disc_variables = disc_model.init(disc_init_rng, dummy_img, train=True)
  ...
  state = custom_train_state.TrainState.create(params=variables['params'], tx=tx)

  if try_checkpoint:
    state, start_epoch = checkpoint.restore_from_path(work_dir, state)
    disc_state, _ = checkpoint.restore_from_path(
        work_dir, disc_state, prefix='disc_ckpt_')
  ...
  p_train_step = jax.pmap(
      functools.partial(train_step, model=model, disc_model=disc_model,
                        config=config),
      axis_name='batch',
  ...
  with metric_writers.ensure_flushes(writer):
    for epoch in range(start_epoch, config.num_epochs + 1):
      # Train part.
      state, disc_state, train_metrics, train_rng = train_epoch(
          p_train_step, state, disc_state, train_ds, config.batch_size, epoch,
          train_rng, kl_tracker_train, config)
      ...
      if is_first_host and epoch % config.save_every == 0:
        ...
        checkpoint.save_checkpoint(
            ...
            keep=10)
        checkpoint.save_checkpoint(
            work_dir, state=flax.jax_utils.unreplicate(disc_state),
            step=epoch + 1, keep=10, prefix='disc_ckpt_')
