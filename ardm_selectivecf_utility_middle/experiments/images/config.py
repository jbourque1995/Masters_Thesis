...
def get_config():
  ...
  config.disc_warmup_epochs = 6  # compatibility alias for older code paths
  config.disc_start_epoch = 6
  config.guidance_ramp_epochs = 20
  ...
  config.align_aux_mode = 'soft'  # off | hard | soft
  config.align_aux_min_cosine = 0.0
  config.align_aux_eps = 1e-8
  config.disc_generator_train = False
  config.cf_mid_weight = 0.0005
  # Lower than the earlier 0.25 scalar-CF test so each dropout event gives a
  # cleaner marginal signal about a smaller subset of channels.
  config.cf_mid_drop_rate = 0.125
  config.cf_mid_delta_clip = 0.05
  config.cf_mid_inverted_dropout = False
  config.cf_mid_credit_mode = 'utility_ema'  # scalar | utility_ema
  config.cf_mid_utility_estimator = 'score'  # score | dropped_only
  config.cf_mid_utility_momentum = 0.99
  config.cf_mid_utility_clip = 0.05
