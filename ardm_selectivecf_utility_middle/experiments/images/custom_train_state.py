...
class TrainState(struct.PyTreeNode):  # pytype: disable=invalid-function-definition  # dataclass_transform
  ...
  # Counterfactual middle-channel utility memory. This is not a trainable model
  # parameter and is not passed through the optimizer. It is an EMA bookkeeping
  # vector used to route feature-credit loss across existing middle channels.
  cf_mid_utility: jnp.ndarray

  # Scalar EMA baseline for the counterfactual delta. This reduces the variance
  # of the score-function channel utility estimator. It is not trainable.
  cf_mid_delta_baseline: jnp.ndarray
  ...
  @classmethod
  def create(cls, *, params, tx, cf_mid_utility=None,
             cf_mid_delta_baseline=None, **kwargs):
    """Creates a new instance with `step=0` and initialized `opt_state`."""
    opt_state = tx.init(params)
    if cf_mid_utility is None:
      cf_mid_utility = jnp.zeros((0,), dtype=jnp.float32)
    if cf_mid_delta_baseline is None:
      cf_mid_delta_baseline = jnp.array(0.0, dtype=jnp.float32)
    return cls(
        step=0,
        ema_params=params,
        cf_mid_utility=cf_mid_utility,
        cf_mid_delta_baseline=cf_mid_delta_baseline,
