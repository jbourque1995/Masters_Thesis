...
from ardm_selectivecf_utility_middle.experiments.images.architectures import unet_utils
from ardm_selectivecf_utility_middle.model.architecture_components import input_embedding
...
class UNet(nn.Module):
  ...
  @nn.compact
  def __call__(self, x, t, mask, train, context=None, return_features=False,
    ...
    if middle_feature_mask is not None:
      # Counterfactual channel dropout is applied after recording the normal
      # middle representation. It changes no parameter count and no tensor shape;
      # it only masks existing channels before the decoder continuation.
      h = h * middle_feature_mask.astype(h.dtype)
