...
from ardm_selectivecf_utility_middle.experiments.images import metric_logging
from ardm_selectivecf_utility_middle.experiments.images.architectures import discriminator
...
from ardm_selectivecf_utility_middle.experiments.images import checkpoint
from ardm_selectivecf_utility_middle.experiments.images import custom_train_state
from ardm_selectivecf_utility_middle.experiments.images import datasets
from ardm_selectivecf_utility_middle.experiments.images.architectures import unet
from ardm_selectivecf_utility_middle.model.autoregressive_diffusion import ao_arm
from ardm_selectivecf_utility_middle.model.autoregressive_diffusion import bit_ao
from ardm_selectivecf_utility_middle.utils import util_fns
...
def _clipped_counterfactual_delta(full_loss, masked_loss, config):
  """Return stop-gradient ARDM-loss delta from a counterfactual feature mask."""
...
def _dropped_channels(channel_mask):
  return (channel_mask <= 1e-6).astype(jnp.float32).reshape((-1,))


def _center_and_normalize_utility(utility, config, eps=1e-6):
  """Convert utility memory into stable signed channel weights."""
  weights = utility.astype(jnp.float32)
  if getattr(config, 'cf_mid_utility_center', True):
    weights = weights - jnp.mean(weights)
  scale = jnp.mean(jnp.abs(weights)) + eps
  weights = weights / scale
  max_abs = getattr(config, 'cf_mid_utility_weight_clip', 3.0)
  return jnp.clip(weights, -max_abs, max_abs)


def counterfactual_channel_utility_loss(fake_features, channel_mask, full_loss,
                                        masked_loss, cf_mid_utility,
                                        cf_mid_delta_baseline, config):
  """Selective counterfactual credit using channel utility memory.

  The old scalar CF loss asked all dropped channels to move together. This loss
  accumulates a per-channel utility estimate:

    delta = masked_ARDM_loss - full_ARDM_loss

  For channels that were dropped:
    * delta > 0 means dropping them hurt, so they were useful.
    * delta < 0 means dropping them helped, so they were harmful/noisy.

  The utility vector is stop-gradient bookkeeping, not a trainable parameter.
  The loss then redistributes existing normalized middle-feature energy toward
  useful channels and away from harmful channels without changing shape, width,
  depth, parameter count, or data.
  """
  delta = _clipped_counterfactual_delta(full_loss, masked_loss, config)
  dropped = _dropped_channels(channel_mask)
  channel_energy = _normalized_channel_energy(fake_features)

  current_credit_mix = getattr(config, 'cf_mid_current_credit_mix', 0.25)
  p_drop = jnp.asarray(getattr(config, 'cf_mid_drop_rate', 0.125), jnp.float32)
  p_drop = jnp.clip(p_drop, 1e-6, 1.0 - 1e-6)
  # Score-function style immediate credit: channels dropped in a bad
  # counterfactual get positive credit; channels kept in that same bad
  # counterfactual get slight negative evidence, and vice versa. This is less
  # biased than assigning the same target only to dropped channels.
  advantage = delta - jax.lax.stop_gradient(cf_mid_delta_baseline)
  current_credit = advantage * (dropped - p_drop) / (p_drop * (1.0 - p_drop))
  utility_for_loss = (jax.lax.stop_gradient(cf_mid_utility.astype(jnp.float32))
                      + current_credit_mix * current_credit)
  utility_weights = _center_and_normalize_utility(utility_for_loss, config)

  # Minimization behavior:
  #   utility > 0: increase relative normalized energy in useful channels.
  #   utility < 0: decrease relative normalized energy in harmful channels.
  # Because channel_energy is RMS-normalized per spatial location, this should
  # redistribute feature emphasis rather than simply inflate all feature norms.
  loss = -jnp.mean(utility_weights * channel_energy)

  drop_count = jnp.maximum(jnp.sum(dropped), 1.0)
  dropped_energy = jnp.sum(channel_energy * dropped) / drop_count
  useful_weight_mean = jnp.mean(jax.nn.relu(utility_weights))
  harmful_weight_mean = jnp.mean(jax.nn.relu(-utility_weights))
  utility_abs_mean = jnp.mean(jnp.abs(cf_mid_utility.astype(jnp.float32)))
  utility_pos_frac = jnp.mean((cf_mid_utility > 0.0).astype(jnp.float32))

  stats = {
      'delta': delta,
      'dropped_energy': dropped_energy,
      'utility_abs_mean': utility_abs_mean,
      'utility_pos_frac': utility_pos_frac,
      'useful_weight_mean': useful_weight_mean,
      'harmful_weight_mean': harmful_weight_mean,
      'utility_max': jnp.max(cf_mid_utility),
      'utility_min': jnp.min(cf_mid_utility),
  }
  return loss, stats


def update_counterfactual_channel_utility(cf_mid_utility, cf_mid_delta_baseline,
                                          channel_mask, delta, config):
  """EMA update for per-channel counterfactual utility.

  This is called after the gradients are computed. It is state bookkeeping only;
  it does not add generator capacity and is not optimized by Adam. For pmap,
  each device may drop different channels, so we aggregate observed credits
  across devices before applying the EMA update.
  """
  dropped = _dropped_channels(channel_mask)
  momentum = getattr(config, 'cf_mid_utility_momentum', 0.99)

  delta_mean = jax.lax.pmean(delta, axis_name='batch')
  new_baseline = (
      momentum * cf_mid_delta_baseline + (1.0 - momentum) * delta_mean)

  estimator = getattr(config, 'cf_mid_utility_estimator', 'score')
  if estimator == 'dropped_only':
    observed = jax.lax.pmean(dropped, axis_name='batch')
    credit_sum = jax.lax.pmean(dropped * delta, axis_name='batch')
    target = credit_sum / jnp.maximum(observed, 1e-6)
    updated = cf_mid_utility + (1.0 - momentum) * observed * (
        target - cf_mid_utility)
  elif estimator == 'score':
    p_drop = jnp.asarray(getattr(config, 'cf_mid_drop_rate', 0.125), jnp.float32)
    p_drop = jnp.clip(p_drop, 1e-6, 1.0 - 1e-6)
    advantage = delta - cf_mid_delta_baseline
    channel_credit = advantage * (dropped - p_drop) / (p_drop * (1.0 - p_drop))
    target = jax.lax.pmean(channel_credit, axis_name='batch')
    updated = cf_mid_utility + (1.0 - momentum) * (target - cf_mid_utility)
  else:
    raise ValueError(
        f'Unknown cf_mid_utility_estimator={estimator!r}; expected score or '
        'dropped_only.')

  utility_clip = getattr(config, 'cf_mid_utility_clip', 0.05)
  updated = jnp.clip(updated, -utility_clip, utility_clip)

  # Optional very mild decay lets stale evidence fade if a channel later stops
  # being useful/harmful. Default 0.0 means no extra decay beyond EMA updates.
  decay = getattr(config, 'cf_mid_utility_decay', 0.0)
  updated = updated * (1.0 - decay)
  return updated, new_baseline
...
def _sanitize_guidance_config(config):
  ...
  if not hasattr(config, 'cf_mid_credit_mode'):
    config.cf_mid_credit_mode = 'utility_ema'
  if config.cf_mid_credit_mode not in ('scalar', 'utility_ema'):
    raise ValueError(
        f'Unknown cf_mid_credit_mode={config.cf_mid_credit_mode!r}; '
        'expected scalar or utility_ema.')
  if not hasattr(config, 'cf_mid_utility_estimator'):
    config.cf_mid_utility_estimator = 'score'
  if config.cf_mid_utility_estimator not in ('score', 'dropped_only'):
    raise ValueError(
        f'Unknown cf_mid_utility_estimator={config.cf_mid_utility_estimator!r}; '
        'expected score or dropped_only.')
  if not hasattr(config, 'cf_mid_utility_momentum'):
    config.cf_mid_utility_momentum = 0.99
  if not hasattr(config, 'cf_mid_utility_clip'):
    config.cf_mid_utility_clip = config.cf_mid_delta_clip
  if not hasattr(config, 'cf_mid_utility_decay'):
    config.cf_mid_utility_decay = 0.0
  if not hasattr(config, 'cf_mid_utility_center'):
    config.cf_mid_utility_center = True
  if not hasattr(config, 'cf_mid_utility_weight_clip'):
    config.cf_mid_utility_weight_clip = 3.0
  if not hasattr(config, 'cf_mid_current_credit_mix'):
...
def train_step(rng, batch, state, disc_mid_state, disc_end_state, epoch, model,
  ...
  def loss_components_fn(params):
    ...
    cf_mid_utility_abs_mean = jnp.array(0.0, dtype=ardm_loss.dtype)
    cf_mid_utility_pos_frac = jnp.array(0.0, dtype=ardm_loss.dtype)
    cf_mid_useful_weight_mean = jnp.array(0.0, dtype=ardm_loss.dtype)
    cf_mid_harmful_weight_mean = jnp.array(0.0, dtype=ardm_loss.dtype)
    cf_mid_utility_max = jnp.array(0.0, dtype=ardm_loss.dtype)
    cf_mid_utility_min = jnp.array(0.0, dtype=ardm_loss.dtype)
    ...
    if use_mid:
      ...
      if use_cf_mid:
        ...
        if getattr(config, 'cf_mid_credit_mode', 'utility_ema') == 'utility_ema':
          cf_mid_loss, cf_stats = counterfactual_channel_utility_loss(
              fake_features['middle'], cf_mid_mask, ardm_loss,
              cf_mid_masked_ardm_loss, state.cf_mid_utility,
              state.cf_mid_delta_baseline, config)
          cf_mid_delta = cf_stats['delta']
          cf_mid_dropped_energy = cf_stats['dropped_energy']
          cf_mid_utility_abs_mean = cf_stats['utility_abs_mean']
          cf_mid_utility_pos_frac = cf_stats['utility_pos_frac']
          cf_mid_useful_weight_mean = cf_stats['useful_weight_mean']
          cf_mid_harmful_weight_mean = cf_stats['harmful_weight_mean']
          cf_mid_utility_max = cf_stats['utility_max']
          cf_mid_utility_min = cf_stats['utility_min']
        else:
          cf_mid_delta = _clipped_counterfactual_delta(
              ardm_loss, cf_mid_masked_ardm_loss, config)
          dropped = _dropped_channels(cf_mid_mask)
          drop_count = jnp.maximum(jnp.sum(dropped), 1.0)
          channel_energy = _normalized_channel_energy(fake_features['middle'])
          cf_mid_dropped_energy = jnp.sum(channel_energy * dropped) / drop_count
    ...
    aux = (ardm_loss, aux_loss, gen_mid_loss, gen_end_loss, reason_mid_loss,
           ...
           cf_mid_masked_ardm_loss, cf_mid_drop_fraction,
           cf_mid_utility_abs_mean, cf_mid_utility_pos_frac,
           cf_mid_useful_weight_mean, cf_mid_harmful_weight_mean,
           cf_mid_utility_max, cf_mid_utility_min,
           disc_weight_mid, disc_weight_end, reason_weight_mid,
           reason_weight_end, cf_mid_weight, guidance_progress, elbo_value,
           elbo_per_t, ce_value, t, fake_features, real_features, feature_info)
    return loss, aux
  ...
  if align_aux_guidance:
    ...
    (ardm_loss, aux_loss, gen_mid_loss, gen_end_loss, reason_mid_loss,
     ...
     cf_mid_useful_weight_mean, cf_mid_harmful_weight_mean,
     cf_mid_utility_max, cf_mid_utility_min,
     disc_weight_mid, disc_weight_end, reason_weight_mid, reason_weight_end,
     cf_mid_weight, guidance_progress, elbo_value, elbo_per_t, ce_value, t,
     fake_features, real_features, feature_info) = aux
  else:
    ...
    (loss, (ardm_loss, aux_loss, gen_mid_loss, gen_end_loss, reason_mid_loss,
            ...
            disc_weight_mid, disc_weight_end, reason_weight_mid,
            reason_weight_end, cf_mid_weight, guidance_progress, elbo_value,
            elbo_per_t, ce_value, t, fake_features, real_features,
            feature_info)), grads = grad_fn(state.params)
  ...
  new_cf_mid_utility = state.cf_mid_utility
  if use_cf_mid and getattr(config, 'cf_mid_credit_mode', 'utility_ema') == 'utility_ema':
    new_cf_mid_utility, new_cf_mid_delta_baseline = (
        update_counterfactual_channel_utility(
            state.cf_mid_utility, state.cf_mid_delta_baseline, cf_mid_mask,
            cf_mid_delta, config))
  else:
    new_cf_mid_delta_baseline = state.cf_mid_delta_baseline

  state = state.apply_gradients(
      grads=grads, cf_mid_utility=new_cf_mid_utility,
      cf_mid_delta_baseline=new_cf_mid_delta_baseline)
  ...
  if use_mid:
    metrics.update({
        ...
        'cf_mid_utility_abs_mean': jax.lax.pmean(
            cf_mid_utility_abs_mean, axis_name='batch'),
        'cf_mid_utility_pos_frac': jax.lax.pmean(
            cf_mid_utility_pos_frac, axis_name='batch'),
        'cf_mid_useful_weight_mean': jax.lax.pmean(
            cf_mid_useful_weight_mean, axis_name='batch'),
        'cf_mid_harmful_weight_mean': jax.lax.pmean(
            cf_mid_harmful_weight_mean, axis_name='batch'),
        'cf_mid_utility_max': jax.lax.pmean(
            cf_mid_utility_max, axis_name='batch'),
        'cf_mid_utility_min': jax.lax.pmean(
            cf_mid_utility_min, axis_name='batch'),
        'cf_mid_delta_baseline': jax.lax.pmean(
            state.cf_mid_delta_baseline, axis_name='batch'),
...
def log_standard_metrics(writer, train_metrics, eval_metrics, epoch):
  ...
  for key in ('ardm_loss', 'gen_mid_loss', 'gen_end_loss',
              ...
              'cf_mid_drop_fraction', 'cf_mid_utility_abs_mean',
              'cf_mid_utility_pos_frac', 'cf_mid_useful_weight_mean',
              'cf_mid_harmful_weight_mean', 'cf_mid_utility_max',
              'cf_mid_utility_min', 'cf_mid_delta_baseline',
...
def train_and_evaluate(config,
  ...
  # Per-channel counterfactual utility is auxiliary bookkeeping, not a
  # generator parameter. Shape equals the existing middle-channel count.
  cf_mid_utility = jnp.zeros((mid_channels,), dtype=jnp.float32)
  state = custom_train_state.TrainState.create(
      params=variables['params'], tx=tx, cf_mid_utility=cf_mid_utility,
      cf_mid_delta_baseline=jnp.array(0.0, dtype=jnp.float32))
