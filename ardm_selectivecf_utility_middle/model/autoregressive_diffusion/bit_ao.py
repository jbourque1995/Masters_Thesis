...
from ardm_selectivecf_utility_middle.model.autoregressive_diffusion import ardm_utils
from ardm_selectivecf_utility_middle.model.autoregressive_diffusion import transitions

from ardm_selectivecf_utility_middle.utils import distribution_utils
from ardm_selectivecf_utility_middle.utils import dynamic_programming
...
class BitUpscaleAutoregressiveDiffusion(struct.PyTreeNode):
  ...
  def elbo_with_features(self, rng, params, x, train, context=None,
                         reference_params=None, reference_train=False,
                         middle_feature_mask=None):
    """ELBO plus internal UNet features for UGARDM discriminator heads.

    `fake_features` are produced by the active/current UNet on the normal
    partially observed ARDM input.

    `real_features` are produced by an EMA/reference UNet. The reference mode is
    configurable through `config.reason_reference_mode`:
      * `mask_matched_ema`: same partial input and same mask as fake branch.
        This removes the trivial partial-vs-fully-observed shortcut and asks the
        current UNet to stay close to the EMA teacher under the same information.
      * `same_mask_full_ema`: full x_future input but the same mask metadata.
      * `full_observed_ema`: old behavior; full input and all-ones mask.
    """
    d = np.prod(x.shape[1:])
    batch_size = x.shape[0]

    rng_perm, rng_t, rng_drop, rng_drop_real, rng_drop_cf = jax.random.split(rng, 5)
    ...
    model_inp = already_predicted * x_future + to_predict * x_past
    fake_net_out, fake_features = self.apply_fn(
        {'params': params}, model_inp, t,
        self.prepare_additional_input(stage, already_predicted), train,
        context, return_features=True,
        rngs={'dropout': rng_drop} if train else None)

    # Reference pass for internal representation guidance. The default is
    # mask-matched EMA teacher features, which removes the easiest discriminator
    # shortcut from the earlier prototype: detecting all-ones mask/full input
    # versus partial mask/partial input.
    if reference_params is None:
      reference_params = params
      reference_train = train

    reference_mode = getattr(self.config, 'reason_reference_mode',
                             'mask_matched_ema')
    if reference_mode == 'mask_matched_ema':
      real_net_inp = model_inp
      real_mask = already_predicted
    elif reference_mode == 'same_mask_full_ema':
      real_net_inp = x_future
      real_mask = already_predicted
    elif reference_mode == 'full_observed_ema':
      real_net_inp = x_future
      real_mask = jnp.ones_like(already_predicted)
    else:
      raise ValueError(
          f'Unknown reason_reference_mode={reference_mode!r}. Expected '
          '`mask_matched_ema`, `same_mask_full_ema`, or `full_observed_ema`.')

    _, real_features = self.apply_fn(
        {'params': reference_params}, real_net_inp, t,
        self.prepare_additional_input(stage, real_mask), reference_train,
        context, return_features=True,
        rngs={'dropout': rng_drop_real} if reference_train else None)

    log_prob_future_given_past = self.log_prob_for_x_future_given_past(
        x_future, x_past, fake_net_out, stage)
    log_prob = log_prob_future_given_past * to_predict
    log_prob = util_fns.sum_except_batch(log_prob)
    ...
    feature_info = {
        'mask_fraction_observed': jnp.mean(already_predicted.astype(jnp.float32)),
        'mask_fraction_to_predict': jnp.mean(to_predict.astype(jnp.float32)),
    }

    if middle_feature_mask is not None:
      cf_net_out = self.apply_fn(
          {'params': params}, model_inp, t,
          self.prepare_additional_input(stage, already_predicted), train,
          context, return_features=False,
          middle_feature_mask=middle_feature_mask,
          rngs={'dropout': rng_drop_cf} if train else None)
      cf_log_prob_future_given_past = self.log_prob_for_x_future_given_past(
          x_future, x_past, cf_net_out, stage)
      cf_log_prob = cf_log_prob_future_given_past * to_predict
      cf_log_prob = util_fns.sum_except_batch(cf_log_prob)
      cf_ce = cf_log_prob / d / np.log(2)
      cf_elbo_per_t = reweighting_factor_expectation_i * cf_log_prob
      cf_elbo = cf_elbo_per_t * reweighting_factor_expectation_t
      feature_info['cf_mid_elbo'] = cf_elbo / d / np.log(2)
      feature_info['cf_mid_ce'] = cf_ce

    return elbo, elbo_per_t, ce, t, fake_features, real_features, feature_info
