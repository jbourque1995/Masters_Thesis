...
from ardm_sampling_constrained_adaptive.model.ardm_constrained_adaptive import ardm_utils
from ardm_sampling_constrained_adaptive.model.ardm_constrained_adaptive import transitions

from ardm_sampling_constrained_adaptive.utils import distribution_utils
from ardm_sampling_constrained_adaptive.utils import dynamic_programming
...
class BitUpscaleAutoregressiveDiffusion(struct.PyTreeNode):
  ...
  def sample_q_t(self, rng, batch_size):
    """Samples a training timestep t and its sampling probability.

    Added modes for fixed-architecture learning-efficiency experiments:
      stratified_t: stratifies timesteps across each batch while preserving the
        original uniform expectation.
    """
    if self.config.elbo_mode in ('uniform', 'adaptive_t', 'constrained_adaptive_t', 'curriculum_t'):
      # Sample t ~ Uniform({0, ..., num_steps-1}). Note randint is exclusive.
      ...
      prob_qt = 1. / self.num_steps
    elif self.config.elbo_mode == 'stratified_t':
      t, prob_qt, stage, t_in_stage = self.sample_stratified_t(
          rng, batch_size)
    else:
      raise ValueError(f'Unknown elbo_mode: {self.config.elbo_mode}')

    return t, prob_qt, stage, t_in_stage

  def sample_stratified_t(self, rng, batch_size):
    """Unbiased stratified timestep sampling over the full ARDM timeline.

    Each batch covers the full range of timesteps more evenly than pure random
    sampling. This changes sampling variance, not model capacity or data.
    """
    rng_jitter, rng_offset = jax.random.split(rng, 2)
    idx = jnp.arange(batch_size)
    offset = jax.random.randint(rng_offset, shape=(), minval=0,
                                maxval=batch_size)
    idx = (idx + offset) % batch_size
    jitter = jax.random.uniform(rng_jitter, shape=(batch_size,))
    t_float = (idx.astype(jnp.float32) + jitter) * (
        float(self.num_steps) / float(batch_size))
    t = jnp.minimum(t_float.astype(jnp.int32), self.num_steps - 1)
    stage = t // self.num_steps_per_stage
    t_in_stage = t % self.num_steps_per_stage
    prob_qt = 1. / self.num_steps
    return t, prob_qt, stage, t_in_stage

  def sample_t_from_policy(self, rng, batch_size, t_policy):
    """Samples t from a full-length probability vector over all timesteps."""
    t_policy = jnp.asarray(t_policy, dtype=jnp.float32)
    t_policy = t_policy / jnp.sum(t_policy)
    logits = jnp.log(jnp.maximum(t_policy, 1e-20))
    t = jax.random.categorical(rng, logits=logits, shape=(batch_size,))
  ...
  def sample_curriculum_t(self, rng, batch_size,
                          min_observed_fraction):
    """Samples t from an easy-to-uniform curriculum over mask fraction.

    min_observed_fraction controls the lower bound on t_in_stage / N. A high
    value starts training on easier states where more variables are observed.
    When it reaches 0.0, the distribution becomes the original uniform over t.
    """
    rng_stage, rng_t = jax.random.split(rng, 2)
    stage = jax.random.randint(rng_stage, shape=(batch_size,), minval=0,
                               maxval=self.num_stages)
    frac = jnp.clip(jnp.asarray(min_observed_fraction, dtype=jnp.float32),
                    0.0, 0.999)
    min_t = jnp.floor(frac * float(self.num_steps_per_stage)).astype(jnp.int32)
    span = self.num_steps_per_stage - min_t
    span = jnp.maximum(span, 1)
    u = jax.random.uniform(rng_t, shape=(batch_size,))
    sampled_offset = jnp.floor(u * span.astype(jnp.float32)).astype(jnp.int32)
    sampled_offset = jnp.minimum(sampled_offset, span - 1)
    t_in_stage = min_t + sampled_offset
    t = t_in_stage + stage * self.num_steps_per_stage
    prob_qt = 1. / (self.num_stages * span.astype(jnp.float32))
    return t, prob_qt, stage, t_in_stage
  ...
  def _elbo_from_sampled_t(self, rng_perm, rng_drop, params, x, train,
                            t, prob_qt, stage, t_in_stage, context=None):
    """Shared ELBO body once t has already been sampled."""
    d = np.prod(x.shape[1:])
  ...
  def elbo(self, rng, params, x, train, context=None):
    ...
    return self._elbo_from_sampled_t(
        rng_perm, rng_drop, params, x, train, t, prob_qt, stage, t_in_stage,
        context=context)

  def elbo_with_t_policy(self, rng, params, x, t_policy, train, context=None):
    """ELBO using a learned/adaptive probability vector over timesteps."""
    batch_size = x.shape[0]
    rng_perm, rng_t, rng_drop = jax.random.split(rng, 3)
    t, prob_qt, stage, t_in_stage = self.sample_t_from_policy(
        rng_t, batch_size, t_policy)
    return self._elbo_from_sampled_t(
        rng_perm, rng_drop, params, x, train, t, prob_qt, stage, t_in_stage,
        context=context)

  def elbo_with_curriculum(self, rng, params, x, min_observed_fraction, train,
                           context=None):
    """ELBO using an easy-to-uniform observed-mask curriculum."""
    batch_size = x.shape[0]
    rng_perm, rng_t, rng_drop = jax.random.split(rng, 3)
    t, prob_qt, stage, t_in_stage = self.sample_curriculum_t(
        rng_t, batch_size, min_observed_fraction)
    return self._elbo_from_sampled_t(
        rng_perm, rng_drop, params, x, train, t, prob_qt, stage, t_in_stage,
        context=context)
